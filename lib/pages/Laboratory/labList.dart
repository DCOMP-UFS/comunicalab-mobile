...
//Define a pagina de lista de Laboratorios
              ...
              if (state is ChoosingStatus) {
                return Row(
                  ...
                  children: <Widget>[
                    statusButton(
                        "Livre",
                        () => BlocProvider.of<FilterlaboratoryBloc>(context)
                            .add(ToStatus(status: "Livre"))),
                    statusButton(
                        "Reservado",
                        () => BlocProvider.of<FilterlaboratoryBloc>(context)
                            .add(ToStatus(status: "Reservado"))),
                    statusButton(
                        "Ocupado",
                        () => BlocProvider.of<FilterlaboratoryBloc>(context)
                            .add(ToStatus(status: "Ocupado"))),
                  ],
                );
              } else {
                return Row(
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (filterState is ByName) {
                      ...
                      if (filterState is ByPlace) {
                        state.labs.forEach((lab) {
                          if (lab['location_id'] != null &&
                              lab['location_id'].contains(controller.text)) {
                            //TODO back não retorna location
                            labs.add(lab);
                          }
                        });
                      } else {
                        if (filterState is ByStatus) {
                          state.labs.forEach((lab) {
                            if (lab['is_in_use'] &&
                                filterState.status == "Ocupado") {
                              //TODO back não retorna status
                              labs.add(lab);
                            }
                          });
                        } else {
    ...
    child: Container(
      ...
      child: Center(
        child: Text(
          text,
          style: TextStyle(
              color: status ? Colors.white : _primaryColor, fontSize: 18),
        ),
      ),
    ),
    onTap: () async {
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Text(labData['location_id'].toString(),
                  style: TextStyle(fontSize: 14)), //TODO integrar com backend
