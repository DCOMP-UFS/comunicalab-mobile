...
//Filtragem listagem de tickets do laboratorio
          ...
          if (state is ChoosingStatus) {
            return Row(
              ...
              children: <Widget>[
                statusButton(
                    "Finalizado",
                    () => BlocProvider.of<FilterticketBloc>(context)
                        .add(ToStatus(status: "Finalizado"))),
                statusButton(
                    "Andamento",
                    () => BlocProvider.of<FilterticketBloc>(context)
                        .add(ToStatus(status: "Andamento"))),
                statusButton(
                    "Pendente",
                    () => BlocProvider.of<FilterticketBloc>(context)
                        .add(ToStatus(status: "Pendente"))),
                statusButton(
                    "Cancelado",
                    () => BlocProvider.of<FilterticketBloc>(context)
                        .add(ToStatus(status: "Cancelado"))),
              ],
            );
          } else {
            return Row(
    ...
    child: BlocBuilder<FilterticketBloc, FilterticketState>(
      builder: (context, filterState) {
        return BlocBuilder<ListTicketBloc, ListTicketState>(
          builder: (context, state) {
            if (BlocProvider.of<ListTicketBloc>(context).state
                is InitialListTicketState) {
              BlocProvider.of<ListTicketBloc>(context)
                  .add(ListTicketLoad(labId: labData['id']));
            }
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            ticketTimeRow(
                DateTime.parse(ticketData["lastProgress"]["progressed_at"])),
    ...
    child: Container(
      ...
      child: Center(
        child: Text(
          text,
          style: TextStyle(
              color: status ? Colors.white : _primaryColor, fontSize: 14),
        ),
      ),
    ),
    onTap: () async {
